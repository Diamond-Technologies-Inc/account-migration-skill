...
7. **Briefs are factual, not narrative.** The `_PROJECT_BRIEF.md` files this skill writes contain provenance, custom instructions (verbatim from export), knowledge inventory, conversation count, and a "what to do next" pointer. They do not synthesize "what's important about this project" — that's the destination-side Claude's job on demand. (Project blueprints, written separately, are the place for narrative synthesis when appropriate.)
